import { HandsPraying, Heart, Sparkle, Coins, Hand } from "@phosphor-icons/react/dist/ssr";
...
const CARDS = [
  ...
  },
  {
    ...
    color: "var(--pink)", bg: "var(--pink-light)",
  ...
  },
  {
    ...
    color: "var(--yellow)", bg: "var(--yellow-light)",
    ...
    <section id="unique" className={s.unique}>
      <div className="container">
        <ScrollReveal className={s.header}>
          ...
          <p className="section-sub">More than a school — a community built on faith, purpose, and a genuine love for every child.</p>
        </ScrollReveal>

        <div className={s.grid}>
          {CARDS.map((card, i) => (
            <ScrollReveal key={card.title} className={s.card} style={{ background: card.bg }} delay={i * 0.08}>
              <div className={s.icon} style={{ color: card.color }}>{card.icon}</div>
              <h4>{card.title}</h4>
              <p>{card.text}</p>
            </ScrollReveal>
          ))}
